...
char* curTime() {
	...
	strftime(buf, sizeof(buf), "%H:%M:%S", &tstruct); // YYYYMMDDHHmmss 형태의 스트링
	//cbuf = LPSTR(buf);
	//char* szLoadImagePath = new char[cbuf.GetLength() + 1];
	return buf;
}
...
void CMFCwebnautesDlg::Running() {
	...
	//GaussianBlur(firstFrame, firstFrame, Size(30, 30), 0);
	...
	while (capture->read(frame)) {
		...
		//GaussianBlur(gray, gray, Size(30, 30), 0);
